...
class Cluster:
    ...
    def refelt(self, reftabix, window=500):
        if self.chrom in reftabix.contigs:
            minpos = self.minpos-500
            if minpos < 0: minpos = 0

            for rec in reftabix.fetch(self.chrom, minpos, self.maxpos+500):
    ...
    def nonref(self, nrtabix, window=500):
        if self.chrom in nrtabix.contigs:
            for rec in nrtabix.fetch(self.chrom, self.minpos-500, self.maxpos+500):
                return '|'.join(rec.strip().split())
...
def build_clusters(bam, chrom=None, window=150, minq=1, minmatch=0.98, cluster_end_plus=0, cluster_end_minus=0):
    ...
    for read in bam_iter:
        if pass_read(read, minq=minq, minmatch=minmatch):
            if read.is_reverse:
                if len(pile_minus) == 0:
                    cluster_end_minus = read.reference_end
                    pile_minus.append(read)

                elif bam.getrname(read.reference_id) != bam.getrname(pile_minus[-1].reference_id) or read.reference_start - window > cluster_end_minus:
                    ...
                    if read.reference_end > cluster_end_minus: cluster_end_minus = read.reference_end
                    pile_minus.append(read)

            else:
                if len(pile_plus) == 0:
                    cluster_end_plus = read.reference_end
                    pile_plus.append(read)

                elif bam.getrname(read.reference_id) != bam.getrname(pile_plus[-1].reference_id) or read.reference_start - window > cluster_end_plus:
                    ...
                    if read.reference_end > cluster_end_plus: cluster_end_plus = read.reference_end
                    pile_plus.append(read)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--window', default=150, help='search window size for next read in cluster (default = 150)')
